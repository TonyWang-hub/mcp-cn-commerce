...
>
> **1926 个测试**全部通过，含对照各平台官方文档的契约测试层。CI 覆盖 Python 3.11/3.12/3.13。
...
| Server | 工具数 | 状态 | 覆盖类别 |
|---|---|---|---|
| taobao | 17 | ✅ 契约已对官方文档核实 | 订单、商品、售后、物流、评价、店铺、营销、类目 |
| xiaohongshu | 16 | ✅ 契约已对官方文档核实 | 订单、商品、售后、物流、库存、财务 |
| weixin_store | 15 | ✅ 契约已对官方文档核实 | 订单、商品、售后、物流、店铺、营销、代发、类目 |
| pinduoduo | 14 | ✅ 契约已对官方文档核实 | 订单、商品、售后、物流、店铺、营销 |
| kuaishou | 13 | ✅ 契约已对官方文档核实 | 订单、商品、售后、评价、店铺、营销 |
| oceanengine | 6 | ⚠️ 待重建 | 仅账户信息与账户余额（其余 16/18 endpoint 已下线或查无此接口） |
| doudian | 4 | ⚠️ 待重建 | 暂无平台工具（20/20 endpoint 不可用） |
| jd | 4 | ⚠️ 待重建 | 暂无平台工具（`jd.pop.*` 命名空间不存在） |
| **合计** | **89** | | 平台工具 57 + 每个 server 4 个通用运维工具 |

> **关于工具数的变化**：本项目对 8 个平台共 115 个 endpoint 做了逐个存在性审计
