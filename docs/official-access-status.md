...
历史初查日期：2026-09-10。范围：本项目的 8 个平台适配器；公开官方网页、官方 SDK、官网公开文档接口。未登录商家或开发者控制台，未使用真实店铺凭证。

## 当前进展（2026-09-11，Core 6b6a7f9）

当前业务范围以 [SDK 逐操作能力表](sdk-integration.md#catalogue-and-evidence-status)和专项合同为准。淘宝、京东、快手后续已取得官方正文并完成相应读合同迁移，不能继续以初查抓取失败描述其最新状态；PDD 已核公共协议，但完整经营业务 schema 仍未取得。XHS 的四个 SDK 原生读操作已核，Source 所需订单查询时间和退款完成时间单位仍有精确缺口。

- 已核订单/售后范围：[抖店](doudian-contract.md)、[TOP](taobao-contract.md)、[京东](jd-contract.md)、[快手](kuaishou-contract.md)、[小红书](xiaohongshu-contract.md)、[其余平台专项记录](remaining-platform-contracts.md)。
- 未闭合合同：[PDD 商家业务 schema](pinduoduo-contract.md)、[XHS/JD 资金采集补证](platform-gap-evidence-20260911.md)；广告报表/账户树和 Pro 主体/委托边界见[集中状态](release-readiness.md)。
- 全部平台尚未完成商家 live。已有准确提交的 CI、包和 MCP 验证见[工程证据](release-readiness.md)，不能替代平台授权和后台核对。

## 历史初查结论（以下保留当时证据）

需要区分三件事：平台开放 API、平台官方 MCP、当前应用实际取得的数据权限。**现有官方 API 不等于已经存在可直接替代本项目的官方 MCP，也不等于用户的账号自动获得全部店铺数据。**

初查阶段已从官方正文确认巨量 Marketing API、抖店商家接入、小红书商家订单 API；快手官网明确提供商家自用和第三方应用及订单能力。京东开放平台入口可访问，但本次未取得协议正文；本地续验已取得淘宝 TOP 签名及订单、微信小店订单列表正文，详见下方续验记录。拼多多、京东和快手的完整当前合同仍未闭合。不能据此宣称这些渠道没有官方 API，也不能宣称八个平台已经全部完成当前协议联调。
...
此节补充并更新上方前一环境记录，未使用商家凭证。官方 HTML 在本机可下载时直接读取正文；网页抓取失败不等于要求登录。

| 平台 | 本次直接来源与实际结果 | 代码处理 / 剩余事项 |
|---|---|---|
| 淘宝 | [TOP 调用协议与固定签名示例](https://developer.alibaba.com/docs/doc.htm?articleId=101617&docType=1)、[卖家订单列表](https://developer.alibaba.com/docs/api.htm?apiId=46)正文已读取；页面未给出明确当前更新时间 | **A（所读合同）**。改为 seller `session`、GMT+8 `yyyy-MM-dd HH:mm:ss`、MD5 包含 `sign_method`，官方固定向量 `66987CB115214E59E6EC978214934FB8` 及 HTTPX 请求回归通过。系统字段在 query。订单每页最多100，按创建时间倒序，限三个月；`payment` 为元。需获批卖家订单权限、有效 session 与真实两页核对。指南和 API 页分别给出 gw.api 与 eco 两个 HTTPS 域名，保留 API 页的 eco 地址。 |
| 微信小店 | [订单列表](https://developers.weixin.qq.com/doc/store/shop/API/channels-shop-order/api_getorderlist.html)完整正文，变更记录 2025-12-03 | **A（订单列表）**。POST `/channels/ec/order/list/get`；create/update_time_range 秒级且最多7天；page_size≤100，使用 next_key/has_more，响应为 order_id_list，需另查详情。已改代码与合成 HTTPX 回归。第三方代调用权限集131，使用 authorizer_access_token；本店使用该小店 access_token。token 自动获取入口及其余业务工具仍待逐项核验。 |
| 京东 | [商家技术规范](https://help.jd.com/oapihelp/question-465.html)可读；宙斯签名工具和零售门户本次仅取得 JS 壳 | 旧官方规范明确 MD5 和 360buy_param_json，与当前 HMAC-MD5/裸 JSON 实现不一致；该文使用旧网关和 v1，不能冒充当前 JOS v2 合同。当前适配器仍待当前官方 SDK/协议和应用权限确认，未按旧文盲改网关。 |
| 拼多多 | 官网 HTML 成功，内容为 JS 应用壳；渲染浏览器打开超时 | 协议正文未取得，仍待官方签名/订单时间单位/商家 ERP 方法、网关、权限。毫秒 timestamp 实现未得到当前官方证明。 |
