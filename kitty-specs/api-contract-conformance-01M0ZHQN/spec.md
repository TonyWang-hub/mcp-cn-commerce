...
**验收标准是"符合官方文档"，不需要真实商家凭证。** 这一点是可达的，因为四个平台提供了官方签名算例，六个平台提供了公开无鉴权的文档接口（见 §4）。
...
## 4. 契约测试框架（WP01，其余 WP 的前置）

不依赖真实凭证的两类验收手段：

### 4.1 官方签名算例（4 个平台可硬断言）
...
- 针对签名的**向量测试**（对 4.1 的平台为硬断言）
- 针对系统参数构造的**wire 断言**：拦截出网请求，校验参数名集合、位置（query/body/header）、timestamp 格式
- 一个**可选的**、默认跳过的联网校验任务，拉取 4.2 的官方 JSON 与本地契约声明比对，用于发现平台侧变更（不进 CI 必过路径，避免外部依赖导致 CI 不稳定）
