...
def _records(shop: dict, source: str, normalizer: Normalizer) -> tuple[list[dict], list[dict], int]:
    ...
    for value in values:
        ...
        invalid_shop_warning = isinstance(warnings, list) and any(
            ...
            and warning.get("field") == "shop_id"
            and warning.get("code") == "identifier_invalid"
            for warning in warnings
        )
        invalid_shop_id = record_shop_id is not None and record_shop_id != "" and not normalized_shop_id
        if (
            value.get("platform") != shop["platform"]
            or invalid_shop_id
            or invalid_shop_warning
            or normalized_shop_id
            and normalized_shop_id != shop["shop_id"]
        ):
...
def _products(orders: list[dict], complete: bool, top_n: int) -> tuple[list[dict], bool, list[dict]]:
    ...
    for order in orders:
        ...
        if any(
            isinstance(w, dict)
            and str(w.get("field", "")).startswith("items")
            and not str(w.get("field", "")).endswith(".sku_id")
        ...
        for item in items:
            ...
            product = products.setdefault(
                product_id,
                {
                    ...
                    "name": str(item.get("product_name") or product_id),
                    "sales": 0,
                    "gross_item_amount": 0,
                    "gmv": None,
                },
...
def _shop_report(shop: dict, days: tuple[str, str], timezone: ZoneInfo, top_n: int) -> dict:
    ...
    errors = [
        _error(
            "upstream_error",
            "input",
            detail=str(e.get("code", "unspecified")) if isinstance(e, dict) else "unspecified",
        )
    ...
    for day in days:
        ...
        refund_ids_valid = all(
            isinstance(refund.get("order_id"), (str, int))
            and not isinstance(refund.get("order_id"), bool)
            and str(refund["order_id"]).strip() not in ("", "None")
            for refund in day_refunds
        )
        ...
        refunded_order_ids = {
            str(refund.get("order_id")) for refund in day_refunds if refund.get("order_id") is not None
        }
        refunded_today_order_count = len(order_ids & refunded_order_ids)
        obs = {
            "order_count": len(day_orders),
            "gmv": sum(amount for amount in paid_amounts if amount is not None),
            "avg_order_value": (
                _average(sum(amount for amount in paid_amounts if amount is not None), len(day_orders))
                if amounts_valid
                else None
            ),
            "refund_count": len(day_refunds),
            "refund_order_count": len(refunded_order_ids) if refund_ids_valid else None,
        ...
        summary = dict(obs)
        metric_complete = {
            "order_count": orders_complete,
            "gmv": orders_complete and amounts_valid,
            "avg_order_value": orders_complete and amounts_valid,
            "refund_count": refunds_complete,
        ...
        completeness[day] = {
            "complete": all(metric_complete.values()),
            "orders": orders_complete,
            "refunds": refunds_complete,
            "metrics": metric_complete,
        }
    top_products, products_complete, product_warnings = _products(
        grouped_orders[days[0]], completeness[days[0]]["orders"], top_n
    )
            ...
            for warning in record.get("warnings", []) if isinstance(record.get("warnings", []), list) else []:
                if isinstance(warning, dict):
                    warnings.append(
                        {
                            "source": source,
                            "record_id": str(record.get("order_id", record.get("refund_id", ""))),
                            "field": str(warning.get("field", "")),
                            "code": str(warning.get("code", "normalization_warning")),
                        }
                    )
    return {
        "platform": shop["platform"],
        "shop_id": str(shop["shop_id"]),
        "shop_name": str(shop.get("shop_name", shop["shop_id"])),
        "summary": summaries[days[0]],
        "yesterday": summaries[days[1]],
        "observed_summary": observed[days[0]],
        "observed_yesterday": observed[days[1]],
        "completeness": completeness,
        "errors": errors,
        "warnings": warnings,
        "top_products": top_products if products_complete else [],
        "observed_top_products": top_products,
...
def _totals(shops: list[dict], key: str) -> dict:
    ...
    totals["refund_rate"] = (
        totals["same_day_refunded_order_count"] / totals["order_count"]
        if totals["same_day_refunded_order_count"] is not None and totals["order_count"]
        else None
    )
    return totals
...
def build_daily_report(date: str, shops: list[dict], *, timezone: str, top_n: int = 3) -> dict[str, Any]:
    ...
    """
    report_date = calendar_date.fromisoformat(date)
    ...
    shops = [
        (
            {
                **shop,
                "platform": normalize_platform(shop.get("platform", "")),
                "shop_id": normalize_identifier(shop.get("shop_id")),
            }
    ...
    reports = [
        _shop_report(shop, days, tz, top_n) for shop in sorted(shops, key=lambda s: (s["platform"], str(s["shop_id"])))
    ]
    return {
        "schema_version": "1.0",
        "date": date,
        ...
        "money_unit": "fen",
        "shops": reports,
        "total_summary": _totals(reports, "summary"),
        "yesterday_summary": _totals(reports, "yesterday"),
